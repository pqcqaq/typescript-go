...
func TestPrimitiveHIRProductionBindingExecutesOnlyCanonicalPhase15Prefix(t *testing.T) {
	...
	if execution.State.Schema != "hir-v6" || !slices.Contains(execution.State.Facts, "typed-hir") {
...
func TestPrimitiveTypedHIRPostVerifierRejectsTampering(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			output := bingo.PassState{Schema: "hir-v6", Facts: []string{"source-type-plan", "typed-hir"}, Artifact: test.mutate(candidate)}
