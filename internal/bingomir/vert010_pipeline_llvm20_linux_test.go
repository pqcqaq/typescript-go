...
func TestExecuteVERT010RunsCanonicalSourceToObjectPipeline(t *testing.T) {
	...
	defer machine.Close()
	result, err := ExecuteVERT010(frontend.Program, identity, plan, machine, runtimeManifest)
	if err != nil {
		t.Fatal(err)
	}
	if result.Replay.HIR.ContentHash != result.MIR.HIRHash || result.BoundMIR.MIR.ContentHash != result.MIR.ContentHash || result.Emission.MIRContentHash != result.BoundMIR.ContentHash {
		t.Fatalf("VERT-010 pipeline provenance did not form one chain: %#v", result)
	}
	if len(result.Emission.Object) < 4 || result.Emission.Object[0] != 0x7f || string(result.Emission.Object[1:4]) != "ELF" {
		t.Fatal("VERT-010 pipeline did not emit an ELF object")
	}
}
