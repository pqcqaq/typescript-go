...
func (oracle *NodeOracle) Add(ctx context.Context, left, right string) (Result, error) {
	...
	command := exec.CommandContext(ctx, oracle.path, "-e", nodeAddScript, "--", left, right)
	...
	return Result{Arguments: []string{left, right}, Output: slices.Clone(output), OutputHash: hashBytes(output)}, nil
}
