...
func TestResolveTargetPassPreservesHIRAndRejectsManifestSubstitution(t *testing.T) {
	...
	hir, err := bingo.NewPassArtifact(bingo.PassArtifactTypedHIR, "hir-v6", []byte(`{"logicalCapabilityRequirements":[]}`))
	if err != nil {
		t.Fatal(err)
	}
	...
	otherHIR, err := bingo.NewPassArtifact(bingo.PassArtifactTypedHIR, "hir-v6", []byte(`{"tamperedButOpaque":true}`))
	if err != nil {
		t.Fatal(err)
	}
