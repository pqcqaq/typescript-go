...
func TestEmitVERT012CommandPublishesVerifiedArtifactSet(t *testing.T) {
	caseDirectory := filepath.Join("..", "..", "testdata", "ts2bin", "closurecounter")
	...
	outputDirectory := filepath.Join(t.TempDir(), "vert012-artifacts")
	arguments := []string{"emit-vert012", "--runtime-manifest", runtimeManifest, "--output-dir", outputDirectory, snapshotPath}
	var stdout, stderr bytes.Buffer
	if code := run(context.Background(), arguments, &stdout, &stderr); code != exitSuccess {
		t.Fatalf("emit-vert012 exit = %d, stdout=%s stderr=%s", code, stdout.String(), stderr.String())
	}
	...
	if _, err := bingo.DecodeClosureContract(read("closure-contract-v1.json")); err != nil {
		t.Fatal(err)
	}
	if _, err := bingo.DecodeVERT012ClosureHIR(read("hir-v11.json")); err != nil {
		t.Fatal(err)
	}
	if _, err := bingo.DecodeObjectLayoutContract(read("cell-layout-v1.json")); err != nil {
		t.Fatal(err)
	}
	if _, err := bingo.DecodeObjectLayoutContract(read("environment-layout-v1.json")); err != nil {
		t.Fatal(err)
	}
	if _, err := bingo.DecodeVERT012MIR(read("mir-v9.json")); err != nil {
		t.Fatal(err)
	}
	if _, err := bingo.DecodeVERT012BoundMIR(read("bound-mir-v1.json")); err != nil {
		t.Fatal(err)
	}
	...
	report, err := decodeVERT012ArtifactReport(read("report.json"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if claimed != hex.EncodeToString(digest[:]) {
		t.Fatal("CLI VERT-012 report hash mismatch")
	}
	stdout.Reset()
	stderr.Reset()
